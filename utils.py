...
def get_all_data_loaders(conf, n_datasets, samples, augmentation, trim,dataset_letters):

    batch_size = conf['batch_size']
    num_workers = conf['num_workers']

    dataset_letters = eval(dataset_letters)
...
def get_model_list(dirname, key, resume):
...
def jaccard(input1, input2):
    ...
    smpInt_cup = input_cup1 & input_cup2
    smpUni_cup = input_cup1 | input_cup2
        
    cntInt_cup = np.count_nonzero(smpInt_cup)
    cntUni_cup = np.count_nonzero(smpUni_cup)
