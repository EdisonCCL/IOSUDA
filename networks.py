...
class UncertaintyDiscriminator(nn.Module):
    def __init__(self, input_dim):
        ...
        self.input_dim=input_dim
        self.conv1 = nn.Conv2d(self.input_dim, filter_num_list[0], kernel_size=4, stride=2, padding=2, bias=False)
...
class AdaINGen2(nn.Module):
    
    # AdaIN auto-encoder architecture.
    def __init__(self, input_dim, params):

        super(AdaINGen2, self).__init__()
...
class UNet5(nn.Module):
    ...
    def forward(self, x, use=None, only_prediction=True):
        ...
        dec4 = self.dec4(torch.cat([center, F.upsample(enc4, center.size()[2:], mode='bilinear')], 1)) # 32 -> 64.
        dec3 = self.dec3(torch.cat([dec4, F.upsample(enc3, dec4.size()[2:], mode='bilinear')], 1)) # 64 -> 128.
        dec2 = self.dec2(torch.cat([dec3, F.upsample(x, dec3.size()[2:], mode='bilinear')], 1)) # 128 -> 256.
        dec1 = self.dec1(dec2) # 256 -> 256.
